...
async def get_characters(destiny_membership_id: int, platform: int):
    ...
    try:
        ...
        profile = await destiny.api.get_destiny_profile(
            destiny_membership_id, platform, [100, 200, 205, 203]
        )
        characters_ids = profile["Response"]["profile"]["data"]["characterIds"]

        # POST guardian to database
        n = profile["Response"]["profile"]["data"]["userInfo"][
            "bungieGlobalDisplayName"
        ]
        c = profile["Response"]["profile"]["data"]["userInfo"][
            ...
            for i in range(len(items_data)):
                ...
                try:
                    if not str(item[i]["bucketHash"]) in no_weapon_buckets:
                        item_raw_data = {}
                        item_resp = await destiny.api.get_item(
                        ...
                        common_data = {"itemHash": i_hash, "bucketHash": b_hash}
...
async def add_guardian_to_db(
    bungie_id: str, name: str, platform: int, decoded_characters_data: dict
):
    try:
        zen = ZENAPI()
        db = SessionLocal()
        ...
        if query is None:  # If guardian does not exists in database add guardian
            ...
            guardian = await zen.api.post_create_guardian(
            ...
            for char, value in decoded_characters_data.items():
                ...
                await zen.api.post_create_character(
                    guardian["id"], payload=char_payload
                )
        else:
            for char, value in decoded_characters_data.items():
                character = (
                    db.query(Character)
                    .filter(
                        Character.guardian_id == query.id, Character.char_class == char
                    )
                    .first()
                )
                if character.last_login != value["dateLastPlayed"]:
                    ...
                    await zen.api.put_update_character(
                        character.id, payload=char_payload
                    )
...
async def main(destiny_membership_ids: int):
    ...
    async with aiohttp.ClientSession():
        ...
        for destiny_membership_id in destiny_membership_ids:
            profiles.append(
                get_characters(destiny_membership_id=destiny_membership_id, platform=3)
            )
